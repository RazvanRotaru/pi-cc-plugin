...
3. If a test reveals an ambiguity in the brief, surface it on the ticket
   rather than making an undocumented call.
4. `commit_checkpoint` and `release_ticket` once the suite is green.

## Constraints

- Don't change the tests. If a test seems wrong, log it and stop —
  surface back to the orchestrator for re-test-review.
- Stay within the ticket's scope. Don't refactor adjacent modules unless
  the brief says so.
